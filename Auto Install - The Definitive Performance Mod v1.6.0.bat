...
echo   %fo1%5}%fw%%preset%%fo0% ::::: %fo1%Only for potato PCs - Playable %fo0%:::: %fw%201%% Boost  
...
echo:
echo   Available fixes (for now):
echo   %fg%You can use  %fw%-  %fr%No effect on this INI%fo1%
echo:
set "fix_use=%fg%"& if not %tdpm_preset%==Performance set "fix_use=[31m"
echo   %fg%%fix_use%1}%fo1% "The grass is shimmering..."
set fix_use=%fg%
echo   %fg%%fix_use%2}%fo1% "The hair of the characters are white..."
set fix_use=%fg%
echo   %fg%%fix_use%3}%fo1% "The [TAB] menu with the characters is lagging..."
echo:
echo:
...
if %tdpm_preset%==Performance (if %whatfix%==1 set ini_variable=r.AOQuality=0 & call %fix_operation% & goto ini_fixes)
if %whatfix%==2 goto ini_fixes_2
if %whatfix%==3 goto ini_fixes_3
