...
set modver=v1.3.2b
set "menusize=mode con:cols=66 lines=44"
chcp 65001 >nul
...
set "msg7=%fg%                    Handheld 45 FPS: Installed                   "
...
echo:
echo     %fo1%[7]%fw% Steam Deck%fo0%:: %fo1%%fob2% [BETA] %blb% Feedbacks are needed%fw%
echo:
echo:
echo     %fob1%%fo1%    INFO    %blb%%fw%
echo     %fo1%[A]%fw% IMPORTANT: USE THIS OPTIMIZED IN-GAME SETTINGS
echo     %fo1%[B]%fw% More details
echo:
echo:
echo     %fob1%%fo1%    IN CASE YOU NEED    %blb%%fw%
echo     %fo1%[C]%fw% Clear + lock Exp33 cache file.
echo     %fo1%[D]%fw% Unlock cache file again.
echo:
echo:
echo     %fo1%[X]%fw% Exit
echo:
...
if %choice%==0 (for %%G in (Engine.ini Scalability.ini) do (del /s /f /q "%cfglocal%\%%G")) & set "msg=%msg0%" & goto menu
::
...
if %choice%==4 (set "inifolder=4_Performance" & set msg=%msg4% & call :installini & call :clearcache & goto menu)
if %choice%==5 (set      "inifolder=5_Potato" & set msg=%msg5% & call :installini & call :clearcache & goto menu)
if %choice%==6 (set "inifolder=6_Monstrosity" & set msg=%msg6% & call :installini & call :clearcache & goto menu)
if %choice%==7 (set "inifolder=7_Steamdeck" & set msg=%msg7% & call :installini & call :clearcache & goto menu)
::
:: Easy-to-read
if /i %choice%==a (goto settings)
if /i %choice%==b (start https://www.nexusmods.com/clairobscurexpedition33/mods/308 & goto menu)
