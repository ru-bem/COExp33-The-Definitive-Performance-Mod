...
:: Use this to enable the upscaler updater if you want. Just change uupdater to 1.
set uupdater=0
...
set "msgi=%fg%                              Applied                            "
set "msgj=%fg%                              Removed                            "
set "upd0=%fg%                     Update DLSS, FSR and XESS                    "
...
set "upd4=%fr%   I'm sorry, but Uupdater isn't compatible with EXP33 v1.5.5+   "
::
...
echo   %fo1%9}%fw% Recommended Settings%fo0%: : %fo1%A}%fw% Update Upscalers%fo0%: : %fo1%B}%fw% INI Fixes  
...
if /i %choice%==a (if %uupdater%==1 (set msg=%upd0% & goto upscaler_updater) else (set msg=%upd4% & goto menu))
...
if /i %choice%==x (if exist "%temp%\DLSS_XESS_FSR" (rd /s /q "%temp%\DLSS_XESS_FSR" >nul 2>&1 & exit) else (exit))
...
:uu_root_or_exe
:: If you pasted root folder, the script will store the dlss, fsr and xess folders as variables and go to update confirmation.
if %glocation%==root (call :uu_set_folders & goto uu_can_update)
:: If you pasted .exe folder, the script will go up three times to the root folder, set the dlss, fsr and xess folders as variables and go to update confirmation.
...
set "fsrfolder=%cd%\Sandfall\Binaries\Win64\"
...
if NOT exist "%fsrfolder%amd_fidelityfx_upscaler_dx12.dll" set msg=%upd2% & goto upscaler_updater
goto:eof
...
echo:
echo   %fo1%1}%fw% Update DlSS, FSR and XESS
...
:: If not present, creates a folder called "DLSS_XESS_FSR" on users temporary folder.
if NOT exist "%temp%\DLSS_XESS_FSR" (md "%temp%\DLSS_XESS_FSR")
echo:

:: Downloads DLSS 4 DLLs from NVIDIA's DLSS github page
echo %fw%    Downloading DLSS 4 DLLs%fo1%[1/2]
curl -# -o "%temp%\DLSS_XESS_FSR\nvngx_dlss.dll" "https://raw.githubusercontent.com/NVIDIA/DLSS/refs/heads/main/lib/Windows_x86_64/rel/nvngx_dlss.dll"
...
:: Downloads XESS DLLs from INTEL's XESS github page
echo %fw%    Downloading XESS DLLs%fo1%[1/4]
curl -# -o "%temp%\DLSS_XESS_FSR\libxess_dx11.dll" "https://raw.githubusercontent.com/intel/xess/refs/heads/main/bin/libxess_dx11.dll"
echo %fw%[2A    Downloading XESS DLLs%fo1%[2/4]
curl -# -o "%temp%\DLSS_XESS_FSR\libxell.dll" "https://raw.githubusercontent.com/intel/xess/refs/heads/main/bin/libxell.dll"
echo %fw%[2A    Downloading XESS DLLs%fo1%[3/4]
curl -# -o "%temp%\DLSS_XESS_FSR\libxess_fg.dll" "https://raw.githubusercontent.com/intel/xess/refs/heads/main/bin/libxess_fg.dll"
echo %fw%[2A    Downloading XESS DLLs%fo1%[4/4]
curl -# -o "%temp%\DLSS_XESS_FSR\libxess.dll" "https://raw.githubusercontent.com/intel/xess/refs/heads/main/bin/libxess.dll"
echo:

:: Downloads FSR DLLs from AMD's FSR github page
echo %fw%    Downloading FSR DLLs%fo1%[1/2]
curl -# -o "%temp%\DLSS_XESS_FSR\amd_fidelityfx_upscaler_dx12.dll" "https://raw.githubusercontent.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK/refs/heads/main/Kits/FidelityFX/signedbin/amd_fidelityfx_upscaler_dx12.dll"
echo %fw%[2A    Downloading FSR DLLs%fo1%[2/2]
curl -# -o "%temp%\DLSS_XESS_FSR\amd_fidelityfx_framegeneration_dx12.dll" "https://raw.githubusercontent.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK/refs/heads/main/Kits/FidelityFX/signedbin/amd_fidelityfx_framegeneration_dx12.dll"
echo:
...
robocopy "%temp%\\DLSS_XESS_FSR\\" "%dlssfolder%\" nvngx_dlss.dll >nul 2>&1
robocopy "%temp%\\DLSS_XESS_FSR\\" "%dlssfolder%\" nvngx_dlssd.dll >nul 2>&1
...
robocopy "%temp%\\DLSS_XESS_FSR\\" "%xessfolder%\" libxe* >nul 2>&1


echo     Updating EXP33 FSR...
:: Verify if there's already an old backup for any of the dlls and deletes it if so.
if exist "%fsrfolder%\amd_fidelityfx_upscaler_dx12.dll.old" (del "%fsrfolder%\amd_fidelityfx_upscaler_dx12.dll.old" >nul 2>&1)
if exist "%fsrfolder%\amd_fidelityfx_framegeneration_dx12.dll.old" (del "%fsrfolder%\amd_fidelityfx_framegeneration_dx12.dll.old" >nul 2>&1)
:: Make a new backup of the DLL installed by adding .old on the end of the file.
ren "%fsrfolder%\amd_fidelityfx_upscaler_dx12.dll" "amd_fidelityfx_upscaler_dx12.dll.old" >nul 2>&1
ren "%fsrfolder%\amd_fidelityfx_framegeneration_dx12.dll" "amd_fidelityfx_framegeneration_dx12.dll.old" >nul 2>&1
:: Copy the FSR DLLs to the correct folder.
robocopy "%temp%\\DLSS_XESS_FSR\\" "%fsrfolder%\" amd_fidelityfx_upscaler_dx12.dll >nul 2>&1
robocopy "%temp%\\DLSS_XESS_FSR\\" "%fsrfolder%\" amd_fidelityfx_framegeneration_dx12.dll >nul 2>&1
...
echo   %fb%2}%fw% Reduce volumetric fog.
echo   %fg%3}%fw% Remove volumetric fog.
echo   %fo1%4}%fw% Increase the view distance.
echo   %fg%5}%fw% EPILEPSY WARNING
echo   %fg%6}%fw% Inverted World (Pretty cool tbh)
echo   %fg%7}%fw% Low Poly World
echo   %fr%8}%fw% Can you can EXP33 now? 💀
echo:
...
if %whatcustom%==1 echo [1A  Applying... & goto ini_custom_1
if %whatcustom%==2 echo [1A  Applying... & goto ini_custom_2
if %whatcustom%==3 echo [1A  Applying... & goto ini_custom_3
if %whatcustom%==4 echo [1A  Applying... & goto ini_custom_4
if %whatcustom%==5 echo [1A  Applying... & goto ini_custom_5
if %whatcustom%==6 echo [1A  Applying... & goto ini_custom_6
if %whatcustom%==7 echo [1A  Applying... & goto ini_custom_7
if %whatcustom%==8 echo [1A  Applying... & goto ini_custom_8
...
for %%G in (
"r.CustomVarAdded=1"
"r.VolumetricFog.VoxelizationShowOnlyPassIndex=5"
) do (set ini_variable=%%~G & call %custom_operation%)
goto ini_custom
